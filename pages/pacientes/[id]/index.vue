        ...
        <form @submit.prevent="editarPaciente" class="space-y-6">
          ...
          <div class="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" v-model="paciente.nombre" @blur="validarCampo('nombre')" required
                class="w-full border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-emerald-400 focus:border-emerald-400 transition" />
            </div>
...
onMounted(async () => {
  try {
    ...
    valoresIniciales.value = {
      ...
      estado_reproductivo: paciente.value.estado_reproductivo || '',
      alimentacion: paciente.value.alimentacion || '',
...
const validarCampo = (campo) => {
  ...
  // Marcar campo como modificado si es opcional y cambió
  if (['color', 'estado_reproductivo', 'alimentacion', 'cirugiasprevias', 'estado'].includes(campo)) {
    if (p[campo] !== valoresIniciales.value[campo]) {
      camposModificados.value[campo] = true;
    }
  }
  ...
  switch (campo) {
    ...
    case 'raza':
      ...
      break;
    case 'edad':
      if (p.edad === null || p.edad === '' || p.edad < 0 || p.edad > 25) {
        error = "La edad debe estar entre 0 y 25 años.";
      }
      break;
    case 'color':
      // Solo validar si el usuario lo modificó
...
const editarPaciente = async () => {
  ...
  const errores = [
    ...
    p.edad === null || p.edad < 0 || p.edad > 30
